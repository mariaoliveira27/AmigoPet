...
<table>
  <tr>
    ...
    <td align="center">
      <div>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 150" width="280px" height="auto">
            <defs>
            ...
            <path d="M 50 85 C 20 85, 20 45, 50 45 C 65 45, 75 55, 80 65 C 85 55, 95 45, 110 45 C 140 45, 140 85, 110 85 C 100 85, 90 95, 80 115 C 70 95, 60 85, 50 85 Z" fill="#38bdf8" />
            <circle cx="45" cy="30" r="12" fill="#475569" />
            <circle cx="80" cy="18" r="14" fill="#475569" />
            <circle cx="115" cy="30" r="12" fill="#475569" />
            <text x="145" y="85" font-family="'Segoe UI', Roboto, Helvetica, Arial, sans-serif" font-size="52" font-weight="900" fill="url(#blueGrad)">
                Amigo<tspan fill="#38bdf8">Pet</tspan>
            </text>
            <text x="150" y="115" font-family="'Segoe UI', Roboto, Helvetica, Arial, sans-serif" font-size="14" font-weight="600" letter-spacing="2" fill="#64748b">
...
O **AmigoPet** nasceu da necessidade urgente de modernizar e centralizar a gestão operacional de Organizações Não Governamentais (ONGs) focadas no resgate e na proteção animal. Muitas dessas instituições enfrentam rotinas exaustivas, dependendo de processos manuais, planilhas descentralizadas ou papéis para controlar a entrada de animais, históricos médicos e processos de adoção. 

O problema central que o AmigoPet resolve é o **gargalo administrativo e a perda de rastreabilidade**. Sem um sistema adequado, as ONGs perdem tempo precioso na triagem de adotantes e no controle de insumos, o que reduz a capacidade de salvar novos animais. O projeto atua como um ecossistema unificado, conectando a linha de frente (voluntários e veterinários) com a administração e o público final (adotantes e doadores).

Para detalhar o impacto da aplicação:

- **Qual foi a ideia inicial?** Criar uma plataforma onde o ciclo de vida do animal resgatado pudesse ser acompanhado de ponta a ponta: desde o registro do resgate nas ruas, passando pelo prontuário clínico e alta veterinária, até a aprovação segura em um novo lar.
- **O que ele entrega de valor ao usuário?** Para a ONG, entrega **eficiência, organização e transparência financeira** (via controle de doações e gestão de estoque). Para o adotante, entrega uma **experiência digital acessível e clara** para buscar e solicitar a adoção de um pet.
- **Onde ele pode ser utilizado?** O sistema é projetado para cenários reais em abrigos de animais, santuários de proteção, ONGs independentes e até Centros de Controle de Zoonoses (CCZ) municipais.
- **Qual o contexto?** O projeto foi concebido com um rigoroso embasamento técnico em Arquitetura de Software e Engenharia de Requisitos, servindo como uma aplicação de referência acadêmica e profissional de alto nível, documentada com diagramação UML e padrões modernos de projeto.
- **Por que contribuir?**Além da relevância social e do impacto direto no bem-estar animal, o projeto é um excelente ambiente para desenvolvedores aplicarem conceitos de sistemas distribuídos, modelagem relacional avançada e padrões MVC.
---
...
A arquitetura do **AmigoPet** foi desenhada com foco em **escalabilidade, segurança e separação de responsabilidades**. O sistema adota o padrão de **Sistemas Distribuídos (Client-Server)**, onde o backend opera como um **Monólito em Camadas (Layered Architecture)**. 

Essa abordagem foi escolhida por proporcionar um excelente balanço entre velocidade de desenvolvimento e facilidade de manutenção para equipes enxutas. Ela resolve o problema do acoplamento excessivo, permitindo que as regras de negócio complexas (como a triagem de adoções) fiquem totalmente isoladas das interfaces de usuário.

---

### 🏢 Visão Geral e Camadas
O backend do sistema é estruturado em três camadas principais, garantindo que o fluxo de informação seja unidirecional e auditável:

1. **Controller Layer (Interface de Comunicação):** Recebe requisições HTTP (REST), valida dados de entrada através de DTOs e repassa o processamento.
2. **Service Layer (Regras de Negócio):** Coração da aplicação. Contém toda a lógica de domínio, como validação de status de animais, geração de relatórios e regras de adoção.
3. **Repository Layer (Acesso a Dados):** Isola a lógica de persistência, interagindo diretamente com o banco de dados relacional via ORM.

### 🧩 Principais Componentes e Papéis

| Componente | Papel no Sistema |
|---|---|
| **Web App (Frontend)** | Interface rica para Adotantes buscarem animais e Administradores gerenciarem a ONG. |
| **Mobile App (Frontend)** | Ferramenta de campo focada na agilidade dos Voluntários (registro de resgates e controle de estoque em tempo real). |
| **API Application** | Servidor central que expõe os endpoints, processa a lógica de negócio e aplica camadas de segurança (autenticação/autorização). |
| **PostgreSQL Database** | Banco de dados relacional que garante a integridade referencial entre animais, prontuários, usuários e adoções. |
| **File Storage (S3)** | Serviço de armazenamento em nuvem dedicado para as fotos dos animais resgatados, evitando sobrecarga no banco de dados. |

---

### 🛠 Padrões de Design Adotados
Para manter o código limpo e extensível, os seguintes padrões foram implementados:

* **Repository Pattern:** Centraliza a lógica de acesso a dados, facilitando a troca do banco de dados no futuro ou a criação de *mocks* para testes unitários.
* **Service Layer:** Evita o padrão "Fat Controller" (controladores inchados), garantindo que os controladores apenas roteiem a requisição, enquanto o serviço toma as decisões de negócio.
* **DTO (Data Transfer Object):** Evita o vazamento de informações sensíveis (como senhas no objeto `Usuario`) ao expor apenas os dados estritamente necessários para o frontend.
* **Single Table Inheritance:** Padrão de ORM aplicado na classe abstrata `Usuario`, onde `Voluntario`, `Veterinario` e `Administrador` são persistidos em uma única tabela para otimizar *queries* e evitar *joins* pesados.

---

### 🔄 Fluxo de Dados (Exemplo: Solicitação de Adoção)
O tráfego de dados na aplicação segue um caminho linear e seguro:

1. O **Adotante** interage com o *Web App*, preenchendo o formulário de intenção.
2. O *Web App* empacota os dados em um arquivo JSON e faz um **POST (HTTPS)** para a *API Application*.
3. O **Controller** recebe a requisição, valida a sintaxe do JSON (DTO) e aciona o **Service**.
4. O **Service** verifica as regras (ex: se o animal está com status `Disponível`), altera o status temporariamente para `Em Análise` e aciona o **Repository**.
5. O **Repository** realiza o *UPDATE/INSERT* no banco de dados **PostgreSQL** dentro de uma transação segura.
6. A confirmação percorre o caminho inverso até exibir a mensagem de sucesso na tela do usuário.

---

### 💻 Tecnologias Utilizadas

| Camada | Tecnologias Principais | Justificativa |
|---|---|---|
| **Frontend Web** | React / Angular | Ecossistema maduro, ideal para Single Page Applications (SPAs) rápidas. |
| **Frontend Mobile** | React Native / Flutter | Desenvolvimento *cross-platform* (Android e iOS) com uma única base de código. |
| **Backend (API)** | Spring Boot (Java) / Node.js | Alta performance e excelente suporte a ORMs robustos (Hibernate / Prisma). |
| **Banco de Dados** | PostgreSQL | SGBD relacional open-source confiável e com forte suporte à integridade de dados (ACID). |
| **Infra/Cloud** | Docker / AWS S3 | Docker garante padronização do ambiente; S3 reduz custos de armazenamento de imagens. |

---

### ⚖️ Decisões Arquiteturais e Trade-offs

* **Monólito Modular vs. Microsserviços:** * *Decisão:* Optamos por iniciar com um monólito em camadas em vez de microsserviços. 
  * *Trade-off:* Embora os microsserviços permitam escalar módulos independentemente (ex: isolar o serviço de relatórios), eles trazem uma grande complexidade de infraestrutura e orquestração. O monólito bem modularizado atende perfeitamente à demanda atual da ONG e mantém os custos de servidor baixos.
* **Mapeamento de Herança (Single Table):**
  * *Decisão:* Unificar todos os tipos de usuários em uma única tabela no banco de dados.
  * *Trade-off:* Gera algumas colunas com valores nulos (ex: `registro_crmv` fica vazio para Voluntários), mas elimina a necessidade de *JOINs* complexos no momento de realizar consultas ou autenticações no sistema.
* **Armazenamento Externo de Imagens:**
  * *Decisão:* Salvar fotos no S3 (Cloud) em vez de convertê-las em *BLOB/Base64* direto no PostgreSQL.
  * *Trade-off:* Adiciona dependência de um serviço de terceiros (rede), mas impede o inchaço rápido do banco de dados, mantendo os backups leves e rápidos.
